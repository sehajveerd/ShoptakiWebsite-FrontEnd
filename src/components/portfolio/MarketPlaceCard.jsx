...
const MyCard = ({ property, cardClassNames }) => {
  // UseNavigate to navigate to the listingpage when a card is clicked.
  // const GET_IMG_URL = gql`
  //   query GetPropertyImages($zpid: Int!) {
  //     getPropertyImages(zpid: $zpid) {
  //       id
  //       zpid
  //       imageURL
  //     }
  ...
  const handleNavigate = () => {
    // Construct the URL with the zpid parameter
    console.log('Navigating to listing page with zpid', property.zpid);
    // const listingPageURL = `/listingpage/${property.zpid}`;

    navigate(`/listingpage/${property.zpid}`);
    // console.log('Navigating to listing page.');
  };

  // Calculate the percentage of investment in relation to the required amount
  const investmentPercentage = (
    (property.renstZestimate / property.price) *
    100
  ).toFixed(2);
  ...
  return (
    // Card container
    <div
      className={`w-[507px] h-[200px] relative ${cardClassNames} cursor-pointer`}
    ...
    >
      ...
      <div className="absolute top-[16px] left-[210px]">
        <div className="font-semibold">{property.street}</div>
        <div className="text-xs text-neutralgray-600">
          {property.city}, {property.stateId} {property.zipcode}
        </div>
      </div>
      ...
      <div className="absolute top-[141px] left-[210px]">
        ...
        {/* <div className="text-sm font-semibold text-neutralgray-700">
          $
          {property.renstZestimate.toLocaleString('en-US', {
            minimumFractionDigits: 2,
          })}
        </div> */}
      </div>
      ...
      <div className="absolute top-[141px] left-[305px]">
        ...
        <div
          className={`text-sm font-semibold text-${getValueColor(
            property.renstZestimate,
            property.price
          )}`}
        >
          {getReturnsPct(property.renstZestimate, property.price)}% {` (`}
          {getFormattedReturns(property.renstZestimate, property.price)}
          {`)`}
        </div>
      </div>
